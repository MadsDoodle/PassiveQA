...
flowchart LR
    ...
    subgraph UNIFIED["§4  Unified Dataset"]
        U["61K samples
action: ANS / ASK / ABS
state.known_vars
state.missing_vars"]
    ...
    subgraph KB["§5  Knowledge Base"]
        K["105K chunks
source-aware chunking
FAISS index"]
    end

    subgraph RAG["§5  RAG Eval — inference ceiling"]
        R1["Baseline RAG
34% acc"]
        R2["Enhanced RAG
34% acc"]
        R3["Decision-aware v3
38% acc"]
    end

    subgraph KG["§6  KG Construction"]
        G0["Phase 1: G₀
NER + dep parsing
27K nodes"]
        G1["Phase 2: G₁
...
2 epochs"]
    ...
    subgraph INFERENCE["§9  Inference Pipeline"]
        Q["Query q
+ V_known"]
        PL["Planner
Mistral-7B + LoRA"]
...
honest refusal"]
    ...
    PL -->|ABSTAIN| AB
    AA & AK & AB --> OUT
    OUT -.->|"resolved var → V_known
multi-turn feedback"| Q
```
